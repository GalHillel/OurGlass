-- OurGlass Supabase schema (snapshot)
-- IMPORTANT: This file must match the app's expected columns, RLS boundaries, and couple scoping.
...
drop policy if exists "Users can see partner's profile" on public.profiles;
create policy "Users can see partner's profile" on public.profiles
  for select using (
...
-- Categories: global + couple-scoped
...
create policy "Everyone can read categories" on public.categories
  for select using (
    auth.role() = 'authenticated' and (couple_id is null or couple_id = public.current_couple_id())
  );

drop policy if exists "Couples can manage their own categories" on public.categories;
create policy "Couples can manage their own categories" on public.categories
  for insert with check (couple_id = public.current_couple_id());

drop policy if exists "Couples can update their own categories" on public.categories;
create policy "Couples can update their own categories" on public.categories
  for update using (couple_id = public.current_couple_id())
  with check (couple_id = public.current_couple_id());

drop policy if exists "Couples can delete their own categories" on public.categories;
create policy "Couples can delete their own categories" on public.categories
  for delete using (couple_id = public.current_couple_id());

-- Couple-scoped tables
drop policy if exists "Couples can read transactions" on public.transactions;
create policy "Couples can read transactions" on public.transactions
  for select using (couple_id = public.current_couple_id());
drop policy if exists "Couples can insert transactions" on public.transactions;
create policy "Couples can insert transactions" on public.transactions
  for insert with check (couple_id = public.current_couple_id());
drop policy if exists "Couples can update transactions" on public.transactions;
create policy "Couples can update transactions" on public.transactions
  for update using (couple_id = public.current_couple_id())
  with check (couple_id = public.current_couple_id());
drop policy if exists "Couples can delete transactions" on public.transactions;
create policy "Couples can delete transactions" on public.transactions
  for delete using (couple_id = public.current_couple_id());

drop policy if exists "Couples can read subscriptions" on public.subscriptions;
create policy "Couples can read subscriptions" on public.subscriptions
  for select using (couple_id = public.current_couple_id());
drop policy if exists "Couples can insert subscriptions" on public.subscriptions;
create policy "Couples can insert subscriptions" on public.subscriptions
  for insert with check (couple_id = public.current_couple_id());
drop policy if exists "Couples can update subscriptions" on public.subscriptions;
create policy "Couples can update subscriptions" on public.subscriptions
  for update using (couple_id = public.current_couple_id())
  with check (couple_id = public.current_couple_id());
drop policy if exists "Couples can delete subscriptions" on public.subscriptions;
create policy "Couples can delete subscriptions" on public.subscriptions
  for delete using (couple_id = public.current_couple_id());

drop policy if exists "Couples can read goals" on public.goals;
create policy "Couples can read goals" on public.goals
  for select using (couple_id = public.current_couple_id());
drop policy if exists "Couples can insert goals" on public.goals;
create policy "Couples can insert goals" on public.goals
  for insert with check (couple_id = public.current_couple_id());
drop policy if exists "Couples can update goals" on public.goals;
create policy "Couples can update goals" on public.goals
  for update using (couple_id = public.current_couple_id())
  with check (couple_id = public.current_couple_id());
drop policy if exists "Couples can delete goals" on public.goals;
create policy "Couples can delete goals" on public.goals
  for delete using (couple_id = public.current_couple_id());

drop policy if exists "Couples can read wishlist" on public.wishlist;
create policy "Couples can read wishlist" on public.wishlist
  for select using (couple_id = public.current_couple_id());
drop policy if exists "Couples can insert wishlist" on public.wishlist;
create policy "Couples can insert wishlist" on public.wishlist
  for insert with check (couple_id = public.current_couple_id());
drop policy if exists "Couples can update wishlist" on public.wishlist;
create policy "Couples can update wishlist" on public.wishlist
  for update using (couple_id = public.current_couple_id())
  with check (couple_id = public.current_couple_id());
drop policy if exists "Couples can delete wishlist" on public.wishlist;
create policy "Couples can delete wishlist" on public.wishlist
  for delete using (couple_id = public.current_couple_id());

drop policy if exists "Couples can read liabilities" on public.liabilities;
create policy "Couples can read liabilities" on public.liabilities
  for select using (couple_id = public.current_couple_id());
drop policy if exists "Couples can insert liabilities" on public.liabilities;
create policy "Couples can insert liabilities" on public.liabilities
  for insert with check (couple_id = public.current_couple_id());
drop policy if exists "Couples can update liabilities" on public.liabilities;
create policy "Couples can update liabilities" on public.liabilities
  for update using (couple_id = public.current_couple_id())
  with check (couple_id = public.current_couple_id());
drop policy if exists "Couples can delete liabilities" on public.liabilities;
create policy "Couples can delete liabilities" on public.liabilities
  for delete using (couple_id = public.current_couple_id());

-- Wealth history: read-only for authenticated couples (write via cron/service role)
drop policy if exists "Couples can read wealth history" on public.wealth_history;
create policy "Couples can read wealth history" on public.wealth_history
  for select using (couple_id = public.current_couple_id());

-- ───────────────────────────────────────────────────────────────────────────────
-- Yield accrual (server-authoritative)
-- ───────────────────────────────────────────────────────────────────────────────
