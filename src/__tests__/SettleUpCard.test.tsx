...
describe('SettleUpCard', () => {
    it('shows loading state initially', () => {
        vi.spyOn(hooks, 'useSettleUp').mockReturnValue({ data: null, isLoading: true } as any);
        const { container } = render(<SettleUpCard />);
        expect(container.querySelector('.animate-pulse')).toBeInTheDocument();
    });

    it('displays settled state when himOwes is near 0', () => {
        vi.spyOn(auth, 'useAuth').mockReturnValue({ profile: { couple_id: '123' } } as any);
    ...
    it('displays amount owed and allows settling', () => {
        ...
        vi.spyOn(query, 'useMutation').mockReturnValue({ mutate: mockMutate, isPending: false } as any);
        vi.spyOn(hooks, 'useSettleUp').mockReturnValue({
